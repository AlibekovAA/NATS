...
func handleStartMessage(m *nats.Msg, partialData map[string][]byte) {
	var start struct {
...
func handleChunkMessage(m *nats.Msg, partialData map[string][]byte) {
	var chunk struct {
		AnalysisID  string `json:"analysis_id"`
...
func handleFinishMessage(m *nats.Msg, partialData map[string][]byte) {
	var finish struct {
		AnalysisID string `json:"analysis_id"`
	}
...
func SubscribeToAnalysis(nc *nats.Conn) error {
	...
	_, err := nc.Subscribe("network.analysis.start", func(m *nats.Msg) {
		handleStartMessage(m, partialData)
	})
	if err != nil {
	...
	_, err = nc.Subscribe("network.analysis.chunk", func(m *nats.Msg) {
		handleChunkMessage(m, partialData)
	})
	if err != nil {
	...
	_, err = nc.Subscribe("network.analysis.finish", func(m *nats.Msg) {
		handleFinishMessage(m, partialData)
	})
	if err != nil {
